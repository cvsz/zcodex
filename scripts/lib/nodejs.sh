...
nodejs_apt_candidate_origin() {
	...
	apt-cache policy nodejs 2>/dev/null | awk '/^[[:space:]]+[0-9]/{in_candidate=1; next} in_candidate && /origin /{print $2; exit}' || true
}
...
nodejs_owner_for_path() {
	local path="$1"
	local real_path nvm_root asdf_root
	...
	/usr/bin/* | /bin/* | /usr/local/bin/*)
		if nodejs_dpkg_package_installed nodejs; then
...
nodejs_runtime_audit() {
	local node_path npm_path node_version npm_version node_owner npm_owner node_count npm_count
...
nodejs_log_runtime_audit() {
	local audit="$1"
	log_info "Runtime audit: mode=$(nodejs_audit_value "${audit}" mode), node=$(nodejs_audit_value "${audit}" node_path) owner=$(nodejs_audit_value "${audit}" node_owner) version=$(nodejs_audit_value "${audit}" node_version), npm=$(nodejs_audit_value "${audit}" npm_path) owner=$(nodejs_audit_value "${audit}" npm_owner)."
...
nodejs_runtime_conflict_report() {
	local audit="$1"
	local mode node_owner npm_owner nvm_detected asdf_detected apt_nodejs nodesource_nodejs distro_npm node_count npm_count node_version
