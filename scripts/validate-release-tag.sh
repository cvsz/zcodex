#!/usr/bin/env bash
# Validate that a release git tag matches the repository VERSION contract.
...
VERSION must contain semantic version X.Y.Z without a leading v.
TAG must be vX.Y.Z and must equal v\$(cat VERSION).
USAGE
}
...
main() {
	...
	printf '[release-tag] OK: %s matches VERSION %s and CHANGELOG.md\n' "${RELEASE_TAG}" "${version}"
}
